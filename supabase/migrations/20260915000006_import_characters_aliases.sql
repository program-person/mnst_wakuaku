-- Applied via MCP apply_migration (name: import_characters_aliases)
-- キャラCSVの取り込みで別名（通称）も登録できるようにする。
-- 戻り値に aliases_added が増えるため、関数を作り直す。
drop function if exists public.import_characters(jsonb);

create function public.import_characters(rows jsonb)
